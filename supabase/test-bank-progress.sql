...
revoke all on table public.test_bank_progress_devices from anon;
grant select, insert, update, delete on table public.test_bank_progress_devices to authenticated;
...
drop policy if exists "Learners delete own test progress" on public.test_bank_progress_devices;
create policy "Learners delete own test progress" on public.test_bank_progress_devices for delete to authenticated using ((select auth.uid()) = user_id);
