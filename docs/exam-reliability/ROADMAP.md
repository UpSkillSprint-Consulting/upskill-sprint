...
| 15 — Handoff | Canonical resumable session with explicit takeover/version ownership; preserve latest cloud-accepted state/deadline; reject stale writes and disclose unuploaded offline limitations. | Implemented on `fix/segment-15-session-handoff`; new PR/human review and database-first rollout pending |
...
Segments 09–14 are human-merged through PRs #176–#180. Segment 15 starts from exact `main` commit `463b844ae1e4e7022cde44b7645b2570b060d38f` and reuses their grading, metrics, history/version, durability, lifecycle, authoritative-timing and incremental-sync contracts. See [Segment 15 canonical session handoff](SEGMENT-15-SESSION-HANDOFF.md). Its additive checkpoint migration must be deployed before the handoff frontend module is enabled. Segment 16 does not start automatically.
