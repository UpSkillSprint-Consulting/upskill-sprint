...
| 15 — Handoff | Canonical resumable session with explicit takeover/version ownership; preserve latest cloud-accepted state/deadline; reject stale writes and disclose unuploaded offline limitations. | Human-merged before Segment 18 entry; cumulative handoff gate retained |
| 16 — New-only | Authoritative, concurrency-safe allocation across eligible exams/modes; explicit reserved/displayed/answered and abandonment policies; no silent Start hangs or readiness gains from reservation alone. | Human-merged before Segment 18 entry; v2 allocation is authoritative |
| 17 — UX/accessibility | Consistent terms/order/scales and error states; manual plus automated responsive, keyboard, screen-reader and agreed WCAG 2.2 AA review; preserve approved visuals/themes/print. | Human-merged in PR #183 at `545315d79b18305137bea22612fc7bfe4f8bb388` |
| 18 — Security/reset | Test actual client roles, grants/RLS/RPCs, cross-account/anonymous/token/stale-device behavior, deletion/reset/export, pending owner data and dependency reachability. No service-role success masquerading as authorization proof. | Implemented for review in PR #184; production migration/Auth configuration verification remains separate |
...
Segments 09–17 are human-merged in the current Segment 18 entry state. Segment 18 starts from exact `main` commit `545315d79b18305137bea22612fc7bfe4f8bb388` and retains their grading, metrics, history/version, durability, lifecycle, authoritative-timing, incremental-sync, handoff, New-only and accessibility contracts. See [Segment 18 security/reset](SEGMENT-18-SECURITY-RESET.md). Its additive database migration must be deployed before the destructive learner-data control is considered production-enabled. Segment 19 does not start automatically.