...
| R08 | Server concurrency implementation and production migration verified / High | Segment 07 adds canonical payload receipts, row-locked session revisions, conflicting-operation rejection, and concurrent-finalization database tests. The production migration, grants/RLS, exact replay, conflict rejection, owner isolation and rollback-only smoke were verified on 2026-09-08. A live two-connection finalization race and later takeover epochs remain unverified. | 07 deployed; retain 15 and 19 for takeover and concurrent production qualification. |
