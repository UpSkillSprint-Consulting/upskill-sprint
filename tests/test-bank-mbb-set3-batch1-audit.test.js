...
test('Q76–175 source remains byte-identical to the audit baseline',()=>{
 const starts=[...source.matchAll(/^  \{$/gm)].map(m=>m.index);assert.equal(starts.length,175);
 assert.equal(crypto.createHash('sha256').update(source.slice(starts[75])).digest('hex'),'1010968ca9290f6d1c473c63f1aeaf6bc2acd687df57846e902503920eaf9a24');
});
