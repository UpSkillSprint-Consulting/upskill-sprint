...
test('final JavaScript trailer source remains byte-identical to the audit baseline',()=>{
 const starts=[...source.matchAll(/^  \{$/gm)].map(m=>m.index);assert.equal(starts.length,175);
 assert.equal(crypto.createHash('sha256').update(source.slice(source.lastIndexOf('  ];'))).digest('hex'),'2d06d874c33463730929799a11508fd6c3bf9402321145fd3cbb09cb2d813f46');
});
