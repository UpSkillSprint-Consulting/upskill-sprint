...
test('Q151–175 source remains byte-identical to the audit baseline',()=>{
 const starts=[...source.matchAll(/^  \{$/gm)].map(m=>m.index);assert.equal(starts.length,175);
 assert.equal(crypto.createHash('sha256').update(source.slice(starts[150])).digest('hex'),'fdbb6f31fbeae47aafea3f47e2ea5d1452d5b5dae00cbfec2e8a32489d9e10d3');
});
