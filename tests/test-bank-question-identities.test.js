...
test('every published question has a literal, unique ID that preserves the prior registry identity', async () => {
  ...
  try {
    ...
    const expectedTotals = { cssbb: 1024, mbb: 475, cssgb: 616, cqe: 933, cmq: 166 };
    let total = 0;
    ...
    assert.equal(total, 3214, 'the complete live question inventory is explicitly identified');
  } finally {
    dom.window.close();
  }
});
