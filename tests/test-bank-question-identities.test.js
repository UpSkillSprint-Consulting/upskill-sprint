...
test('every published question has a literal, unique ID that preserves the prior registry identity', async () => {
  ...
  try {
    ...
    const expectedTotals = { cssbb: 1024, mbb: 125, cssgb: 616, cqe: 933, cmq: 166 };
    let total = 0;
    ...
    assert.equal(total, 2864, 'the complete live question inventory is explicitly identified');
  } finally {
    dom.window.close();
  }
});
