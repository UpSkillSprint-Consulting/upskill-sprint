...
test('MBB 160 Batch 1 has the approved 25-question allocation and is published as a clearly partial Set 2', () => {
  ...
  assert.match(pageSource, /sets:\{1:MBB_SET1,2:MBB_SET2,3:MBB_SET3\}/, 'the validated batch is registered as MBB Set 2 alongside Set 3');
});
...
test('MBB Set 2 aggregates validated batches and launches only the currently available questions', async () => {
  ...
  try {
    ...
    assert.match(set2Button.textContent, /125 of 160/i);
    assert.match(set2Button.textContent, /Batch 5 complete/i);
    ...
    assert.match(overview.textContent, /contains 125 of the planned 160 original questions/i);
    assert.match(overview.textContent, /current Full Exam serves all 125 available questions/i);
    const fullCard = overview.querySelector('.tb-mode');
    assert.match(fullCard.querySelector('h4').textContent, /Set 2 — Full Exam/i);
    assert.match(fullCard.textContent, /125 randomized questions/i);
    assert.match(fullCard.textContent, /Strict 3 hr 7 min 30 sec limit/i);

    click(overview.querySelector('[data-mode="full"]'));
    assert.equal(overview.querySelectorAll('.tb-navcell').length, 125);
