...
test('MBB Set 2 aggregates validated batches and launches only the currently available questions', async () => {
  ...
  try {
    ...
    assert.match(set2Button.textContent, /75 of 160/i);
    assert.match(set2Button.textContent, /Batch 3 complete/i);
    ...
    assert.match(overview.textContent, /contains 75 of the planned 160 original questions/i);
    assert.match(overview.textContent, /current Full Exam serves all 75 available questions/i);
    const fullCard = overview.querySelector('.tb-mode');
    assert.match(fullCard.querySelector('h4').textContent, /Set 2 — Full Exam/i);
    assert.match(fullCard.textContent, /75 randomized questions/i);
    assert.match(fullCard.textContent, /Strict 1 hr 52 min 30 sec limit/i);

    click(overview.querySelector('[data-mode="full"]'));
    assert.equal(overview.querySelectorAll('.tb-navcell').length, 75);
