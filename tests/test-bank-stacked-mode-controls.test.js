...
test('mistake notebook stays mounted while rapid filter changes keep both Set rows anchored', async () => {
  ...
  filter.focus();
  filter.dispatchEvent(new window.Event('click', { bubbles: true }));
  assert.equal(window.document.activeElement.getAttribute('data-missed'), 'quick', 'the newest filter action owns focus');
  await new Promise(resolve => window.setTimeout(resolve, 0));
  await settle(window, 3);
  assert.equal(window.document.activeElement.getAttribute('data-missed'), 'quick', 'background synchronization cannot steal focus from Missed-only');
