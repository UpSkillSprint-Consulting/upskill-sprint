...
class Clock {
  ...
  install(w) {
    ...
    // Ledger tests use explicit sync and never start UI intervals.
...
class Service {
  ...
  async exchange(owner, request) {
    ...
    if (request.kind === 'upsert') {
      ...
      for (const row of request.batch) {
        assert.equal(row.user_id, owner, 'transport owner mismatch');
        const key = owner + '|' + row.event_id;
        if (!this.rows.has(key)) this.rows.set(key, {...copy(row),received_at:new Date(EPOCH + ++this.sequence).toISOString()});
      }
    ...
    if (request.kind === 'read') {
      ...
      rows = rows.slice(request.from || 0, request.to == null ? undefined : request.to+1);
...
function client(send) {
  return { rpc(name,args) { return send({kind:'rpc',name,args}); }, from(table) { return {
    ...
    select() {
      ...
      const q = {
        eq(key,value) { if(key === 'user_id')state.owner=value; return q; },
        gte(key,value) { if(key === 'received_at')state.since=value; return q; },
        order() {return q;},
        abortSignal() {return q;},
        range(from,to) { return send({...state,from,to}); },
        limit(count) {return send({...state,from:0,to:count-1});},
        maybeSingle() {return send(state).then(r=>({...r,data:Array.isArray(r.data)?r.data[0]||null:r.data}));},
        then(resolve,reject) {return send(state).then(resolve,reject);}
      }; return q;
    }
  }; }};
}
function mutate(source, name) {
  const variants = {
    counting: ['answeredEvents: answeredEvents,', 'answeredEvents: answeredEvents + 1,'],
    sync: ['mergeRemoteRows(state, remoteRows, userId);', 'mergeRemoteRows(state, [], userId);']
  };
  if (!name) return source;
  assert.ok(Object.hasOwn(variants,name),'Unknown mutation');
  const [from,to] = variants[name]; assert.equal(source.split(from).length,2,'Mutation seam changed');
...
function device(service, options = {}) {
  ...
  const dom = new JSDOM('<!doctype html><body></body>', {url:'https://segment03.invalid/test-bank',runScripts:'outside-only'});
  const w = dom.window, clock = new Clock(); clock.install(w);
  Object.defineProperty(w.document,'readyState',{get:()=> 'loading',configurable:true}); // no background initialization
  let online = true, user = owner, denied = false;
  Object.defineProperty(w.navigator,'onLine',{get:()=>online,configurable:true});
  w.fetch = () => {throw new Error('External network forbidden');};
  for (const [k,v] of Object.entries(storage)) w.localStorage.setItem(k,v);
  w.localStorage.setItem('tb-account-sync-device-v1',id);
  const set = w.Storage.prototype.setItem;
  w.Storage.prototype.setItem = function(k,v) { if(denied && k==='tb-learning-events-v2')throw new w.DOMException('Synthetic quota','QuotaExceededError'); return set.call(this,k,v); };
  w.UpskillAuth = {getUser:()=>user ? {id:user}:null,getClient:()=>service.client(user)};
  const qs=questions(exam);
  w.__TB={EXAMS:{[exam]:{questions:165,sets:{1:qs},bok:[{subs:qs.map((q,i)=>({id:q.sub,name:q.sub,w:i+1}))}]}}};
  w.eval(fs.readFileSync(path.join(ROOT,'test-bank-question-registry.js'),'utf8'));
  w.eval(mutate(fs.readFileSync(path.join(ROOT,'test-bank-learning-events.js'),'utf8'),mutation));
  return {w,clock,questions:qs,api:w.__TBLearning,close:()=>dom.window.close(),
    setOnline:v=>{online=v;},setOwner:v=>{user=v;},denyStorage:v=>{denied=v;},
    snapshot:()=>Object.fromEntries(Array.from({length:w.localStorage.length},(_,i)=>w.localStorage.key(i)).map(k=>[k,w.localStorage.getItem(k)]))};
}
function completed(d, sessionId='fixture-session-1') {
  const q=d.questions, exam=q[0].qid.split(':')[0];
  d.api.startSession({examId:exam,sessionId,questions:q,mode:'quick',timed:false});
  return d.api.completeSession({examId:exam,sessionId,records:[{question:q[0],selected:0,status:'correct'},{question:q[1],selected:0,status:'incorrect'},{question:q[2],selected:null,status:'unanswered'}]});
}
