...
test('Batch 6 numerical alternatives remain exact and accessible without the optional graphical module',()=>withPage(w=>{
  assert.equal(w.__MBBSet3Batch6UI,undefined);
  const items=w.__TB.EXAMS.mbb.sets[3].slice(125,150).filter(q=>q.chart&&q.chart.evidence);assert.equal(items.length,5);
...
test('The normal player still uses all five graphical Batch 6 visuals, not their degraded-mode tables alone',()=>withPage(w=>{
  w.eval(read('test-bank-mbb-set3-batch6-ui.js'));
  const items=w.__TB.EXAMS.mbb.sets[3].slice(125,150).filter(q=>q.chart&&q.chart.evidence);assert.equal(items.length,5);
  for(const q of items)for(const review of [false,true]){
    const rendered=w.__TB.renderQuestionContent(q,review);
    assert.match(rendered,/mbbs3b6-question/);assert.match(rendered,/<svg/);assert.doesNotMatch(rendered,/tb-chart-evidence-fallback|NaN|undefined/);
