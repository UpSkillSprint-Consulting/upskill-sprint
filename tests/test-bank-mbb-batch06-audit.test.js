...
test('150 unaffected records, 24 other assets and 174 keys remain exact; only Q150 is rekeyed',()=>{
 const p=JSON.parse(read('docs/audits/mbb-set2-batch06/preservation.json'));assert.equal(all.length,175);assert.equal(Object.keys(p.questions_sha256).length,150);assert.equal(Object.keys(p.asset_sha256).length,24);
 for(const q of all)if(q.batch!==6)assert.equal(hash(JSON.stringify(stable(q))),p.questions_sha256[q.qid],q.qid);
 for(const [file,value]of Object.entries(p.asset_sha256))assert.equal(hash(fs.readFileSync(path.join(root,file))),value,file);
