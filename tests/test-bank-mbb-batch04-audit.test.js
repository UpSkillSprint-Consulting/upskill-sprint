...
test('Prior Batch 4 preservation plus explicitly integrated Batch 3 hashes remain intact',()=>{
 ...
 for(const [qid,sha] of Object.entries(manifest.question_sha256))assert.equal(digest(JSON.stringify(all.find(q=>q.qid===qid))),batch5Updated.question_sha256[qid] || restored.question_sha256[qid] || sha,qid);
 assert.equal(Object.keys(manifest.asset_sha256).length,24);
 for(const [p,sha] of Object.entries(manifest.asset_sha256))assert.equal(digest(fs.readFileSync(path.join(root,p))),batch5Updated.asset_sha256[p] || restored.asset_sha256[p] || sha,p);
});
