...
test('CSSBB carries a third 165-question set with the BoK distribution', async () => {
  const w = await loadPage();
  const e = w.__TB.EXAMS.cssbb;
  assert.ok(e.sets[3], 'Set 3 registered');
  assert.equal(e.sets[3].length, 165);
  ...
  assert.equal(new Set(e.sets[3].map(q => q.stem)).size, 165, 'all Set-3 stems unique');
});
...
test('a full exam is 165 for Set 3 and for the all-sets Mixed pool', async () => {
  ...
  assert.equal(await count('mix'), 165, 'Mixed full exam is 165 drawn from the 495 pool');
});
