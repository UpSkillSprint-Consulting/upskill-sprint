"""Secondary Batch 7 checks: no-JS fallbacks, dark/mobile and 320px layouts.
Run: python tests/browser/mbb-batch07-secondary.py --out /tmp/mbb-secondary
Reuses the actual source/fixture loader from mbb-batch07-render.py; no live writes.
"""
...
def inspect_evidence(page,scope,label):
    ...
    for el in scope.locator('.mbb7-scroll').all():
    ...
    check(scope.locator('.mbb7-table th,.mbb7-table td').evaluate_all('els=>els.every(e=>parseFloat(getComputedStyle(e).fontSize)>=14)'),label+' readable table typography')
import hashlib
...
results['static_fallback_sha256']=hashlib.sha256((ROOT/'test-bank-assets/mbb-160/batch-07/static-fallbacks.html').read_bytes()).hexdigest()

with sync_playwright() as p:
    browser=p.chromium.launch(executable_path='/usr/bin/chromium',headless=True,args=['--no-sandbox'])
    fallback=re.sub(r'<script\b[^>]*>[\s\S]*?</script>','',read('test-bank-assets/mbb-160/batch-07/static-fallbacks.html'))
    for theme in ['light','dark']:
        for device,width,height in [('desktop',1440,1000),('tablet',768,1024),('mobile',390,844)]:
            ...
            check(page.locator('select,[data-mbb7-point]').count()==0,f'{theme} {device} no dead fallback controls')
            for card in page.locator('.fallback-card').all():
                ...
                check(card.locator('.mbb7-conditions').count()==1,label+' assumptions')
    ...
    for theme,width,label in [('dark',390,'dark-mobile'),('light',320,'narrow-mobile')]:
        ...
        for i,key in enumerate(KEYS):
            page.locator(f'[data-goto="{i}"]').click();no_overflow(page,f'{label} Q{i+151} attempt page')
            inspect_evidence(page,page.locator('#tb-overview'),f'{label} Q{i+151}')
            if theme=='dark':check(page.locator('.tb-qtag').evaluate('(el)=>getComputedStyle(el).color===getComputedStyle(document.querySelector(".tb-stem")).color'),f'{label} Q{i+151} readable topic ink')
            page.locator(f'[data-opt="{key}"]').click()
            screenshot_element(page,'#tb-overview',OUT/f'{label}-attempt-{i+151}.png')
            results['screens'].append({'theme':theme,'device':label,'question':i+151,'mode':'attempt'})
        ...
        for i in range(25):
            ...
            card=page.locator('.tb-review-card');inspect_evidence(page,card,f'{label} review Q{i+151}')
            no_overflow(page,f'{label} Q{i+151} review page')
            check(card.locator('.mbb7-rationales dt').count()==4,f'{label} Q{i+151} four rationales')
            screenshot_element(page,'.tb-review-card',OUT/f'{label}-review-{i+151}.png')
            results['screens'].append({'theme':theme,'device':label,'question':i+151,'mode':'review'})
        page.close()
    browser.close()
