"""Integration smoke test for all five audited batches on the actual player.
...
keys=['ABCD'.index(r['verified_key']) for r in b1]+['ABCD'.index(r['correct_letter']) for r in b2]+['ABCD'.index(r['key']) for r in b3]+['ABCD'.index(r['independent_answer']) for r in b4]+KEYS
assert len(keys)==125
results={'environment':'Actual source; offline Chromium auth/memory fixtures; 125-question integration, not a new source-page audit','assertions':[],'screens':[],'errors':[]}
...
with sync_playwright() as p:
 ...
 for label,width,height in [('desktop',1440,1000),('tablet',768,1024),('mobile',390,844)]:
  ...
  page.evaluate('()=>{window.__TB.EXAMS.mbb.sets[2]=[1,2,3,4,5].flatMap(b=>window.MBB_SET2_BATCHES[b]);Math.random=()=>0.9999999}')
  page.locator('[data-mode="full"]').click();check(page.locator('.tb-navcell').count()==125,label+' 125-question contiguous sequence')
  for i,key in enumerate(keys):
   ...
   check(page.locator('.mbb2-rationales,.mbb3-rationales,.mbb4-rationales,.mbb5-rationales').count()==0,f'{label} Q{n} no premature rationales')
   page.locator(f'[data-opt="{key}"]').click()
   results['screens'].append({'device':label,'question':n,'mode':'attempt','capture':False})
  for n in [1,25,26,50,51,75,76,100,101,125]:
   page.locator(f'[data-goto="{n-1}"]').click();check(page.locator(f'[data-opt="{keys[n-1]}"].sel').count()==1,f'{label} Q{n} cross-batch retention')
  page.locator('[data-goto="124"]').click();page.locator('[data-submit]').click();page.wait_for_timeout(100)
  check('125 of 125 correctly' in page.locator('#tb-overview').inner_text(),label+' recorded independent answers score 125/125')
  ...
  for n in range(1,126):
   ...
   if n in [5,36,61,84,105]:screenshot_element(page,'.tb-review-card',OUT/f'{label}-review-{n}.png')
   results['screens'].append({'device':label,'question':n,'mode':'review','capture':n in [5,36,61,84,105]})
  # One mixed retry queue proves all five renderer paths coexist.
  retry=[5,36,61,84,105];page.set_content(raw,wait_until='load');page.locator('.tb-tile[data-exam="mbb"]').click();page.locator('.tb-setpick [data-set="2"]').click()
  page.evaluate('(ids)=>{window.__TB.EXAMS.mbb.sets[2]=[1,2,3,4,5].flatMap(b=>window.MBB_SET2_BATCHES[b]).filter(q=>ids.includes(Number(q.qid.slice(-3))));Math.random=()=>0.9999999}',retry)
