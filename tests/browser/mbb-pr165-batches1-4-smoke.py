"""Integration smoke test for all four audited batches on the actual player.
...
loader=Path(__file__).with_name('mbb-batch03-render.py')
...
keys=['ABCD'.index(r['verified_key']) for r in b1]+['ABCD'.index(r['correct_letter']) for r in b2]+KEYS+['ABCD'.index(r['independent_answer']) for r in b4]
assert len(keys)==100
results={'environment':'Actual source; offline Chromium auth/memory fixtures; 100-question integration, not a new source-page audit','assertions':[],'screens':[],'errors':[]}
...
with sync_playwright() as p:
 ...
 for label,width,height in [('desktop',1440,1000),('tablet',768,1024),('mobile',390,844)]:
  ...
  page.evaluate('()=>{window.__TB.EXAMS.mbb.sets[2]=[1,2,3,4].flatMap(b=>window.MBB_SET2_BATCHES[b]);Math.random=()=>0.9999999}')
  page.locator('[data-mode="full"]').click();check(page.locator('.tb-navcell').count()==100,label+' 100-question contiguous sequence')
  for i,key in enumerate(keys):
   ...
   check(page.locator('.mbb2-rationales,.mbb3-rationales,.mbb4-rationales').count()==0,f'{label} Q{n} no premature rationales')
   page.locator(f'[data-opt="{key}"]').click()
   results['screens'].append({'device':label,'question':n,'mode':'attempt','capture':False})
  for n in [1,25,26,50,51,75,76,100]:
   page.locator(f'[data-goto="{n-1}"]').click();check(page.locator(f'[data-opt="{keys[n-1]}"].sel').count()==1,f'{label} Q{n} cross-batch retention')
  page.locator('[data-goto="99"]').click();page.locator('[data-submit]').click();page.wait_for_timeout(100)
  check('100 of 100 correctly' in page.locator('#tb-overview').inner_text(),label+' recorded independent answers score 100/100')
  ...
  for n in range(1,101):
   ...
   if n in [5,36,61,84]:screenshot_element(page,'.tb-review-card',OUT/f'{label}-review-{n}.png')
   results['screens'].append({'device':label,'question':n,'mode':'review','capture':n in [5,36,61,84]})
  # One mixed retry queue proves all four renderer paths coexist.
  retry=[5,36,61,84];page.set_content(raw,wait_until='load');page.locator('.tb-tile[data-exam="mbb"]').click();page.locator('.tb-setpick [data-set="2"]').click()
  page.evaluate('(ids)=>{window.__TB.EXAMS.mbb.sets[2]=[1,2,3,4].flatMap(b=>window.MBB_SET2_BATCHES[b]).filter(q=>ids.includes(Number(q.qid.slice(-3))));Math.random=()=>0.9999999}',retry)
