"""Secondary Batch 4 checks: no-JS fallbacks, dark/mobile and 320px layouts.
Run: python tests/browser/mbb-batch04-secondary.py --out /tmp/mbb-secondary
Reuses the actual source/fixture loader from mbb-batch04-render.py; no live writes.
"""
...
loader=Path(__file__).with_name('mbb-batch04-render.py')
...
def inspect_evidence(page,scope,label):
    ...
    for el in scope.locator('.mbb4-scroll').all():
    ...
    check(scope.locator('.mbb4-table th,.mbb4-table td').evaluate_all('els=>els.every(e=>parseFloat(getComputedStyle(e).fontSize)>=14)'),label+' readable table typography')
with sync_playwright() as p:
    browser=p.chromium.launch(executable_path='/usr/bin/chromium',headless=True,args=['--no-sandbox'])
    fallback=re.sub(r'<script\b[^>]*>[\s\S]*?</script>','',read('test-bank-assets/mbb-160/batch-04/static-fallbacks.html'))
    for theme in ['light','dark']:
        for device,width,height in [('desktop',1440,1000),('tablet',768,1024),('mobile',390,844)]:
            ...
            check(page.locator('select,[data-mbb4-point]').count()==0,f'{theme} {device} no dead fallback controls')
            for card in page.locator('.fallback-card').all():
                ...
                check(card.locator('.mbb4-conditions').count()==1,label+' assumptions')
                check(card.locator('table caption').count()>0,label+' data alternative')
                if n==99:check(card.locator('.mbb4-matrices tbody tr').count()==40,label+' all 40 run rows')
    ...
    for theme,width,label in [('dark',390,'dark-mobile'),('light',320,'narrow-mobile')]:
        ...
        for i,key in enumerate(KEYS):
            page.locator(f'[data-goto="{i}"]').click();no_overflow(page,f'{label} Q{i+76} attempt page')
            inspect_evidence(page,page.locator('#tb-overview'),f'{label} Q{i+76}')
            page.locator(f'[data-opt="{key}"]').click()
            screenshot_element(page,'#tb-overview',OUT/f'{label}-attempt-{i+76}.png')
            results['screens'].append({'theme':theme,'device':label,'question':i+76,'mode':'attempt'})
        ...
        for i in range(25):
            ...
            card=page.locator('.tb-review-card');inspect_evidence(page,card,f'{label} review Q{i+76}')
            no_overflow(page,f'{label} Q{i+76} review page')
            check(card.locator('.mbb4-rationales dt').count()==4,f'{label} Q{i+76} four rationales')
            if i+76==87:
                check('0.80' in card.locator('.tb-key-point').inner_text(),label+' rendered learning point preserves 0.80')
                check('3.00' in card.inner_text(),label+' complete explanation retains 3.00')
            screenshot_element(page,'.tb-review-card',OUT/f'{label}-review-{i+76}.png')
            results['screens'].append({'theme':theme,'device':label,'question':i+76,'mode':'review'})
        page.close()
    browser.close()
