"""Secondary Batch 5 checks: no-JS fallbacks, dark/mobile and 320px layouts.
Run: python tests/browser/mbb-batch05-secondary.py --out /tmp/mbb-secondary
Reuses the actual source/fixture loader from mbb-batch05-render.py; no live writes.
"""
...
def inspect_evidence(page,scope,label):
    ...
    for el in scope.locator('.mbb5-scroll').all():
    ...
    check(scope.locator('.mbb5-table th,.mbb5-table td').evaluate_all('els=>els.every(e=>parseFloat(getComputedStyle(e).fontSize)>=14)'),label+' readable table typography')
with sync_playwright() as p:
    browser=p.chromium.launch(executable_path='/usr/bin/chromium',headless=True,args=['--no-sandbox'])
    fallback=re.sub(r'<script\b[^>]*>[\s\S]*?</script>','',read('test-bank-assets/mbb-160/batch-05/static-fallbacks.html'))
    for theme in ['light','dark']:
        for device,width,height in [('desktop',1440,1000),('tablet',768,1024),('mobile',390,844)]:
            ...
            check(page.locator('select,[data-mbb5-point]').count()==0,f'{theme} {device} no dead fallback controls')
            for card in page.locator('.fallback-card').all():
                ...
                check(card.locator('.mbb5-conditions').count()==1,label+' assumptions')
    ...
    for theme,width,label in [('dark',390,'dark-mobile'),('light',320,'narrow-mobile')]:
        ...
        for i,key in enumerate(KEYS):
            page.locator(f'[data-goto="{i}"]').click();no_overflow(page,f'{label} Q{i+101} attempt page')
            inspect_evidence(page,page.locator('#tb-overview'),f'{label} Q{i+101}')
            if theme=='dark':check(page.locator('.tb-qtag').evaluate('(el)=>getComputedStyle(el).color===getComputedStyle(document.querySelector(".tb-stem")).color'),f'{label} Q{i+101} readable topic ink')
            page.locator(f'[data-opt="{key}"]').click()
            screenshot_element(page,'#tb-overview',OUT/f'{label}-attempt-{i+101}.png')
            results['screens'].append({'theme':theme,'device':label,'question':i+101,'mode':'attempt'})
        ...
        for i in range(25):
            ...
            card=page.locator('.tb-review-card');inspect_evidence(page,card,f'{label} review Q{i+101}')
            no_overflow(page,f'{label} Q{i+101} review page')
            check(card.locator('.mbb5-rationales dt').count()==4,f'{label} Q{i+101} four rationales')
            if i+101==114:
                check('−$13,636.36' in card.inner_text(),label+' complete NPV explanation retains decimals')
            screenshot_element(page,'.tb-review-card',OUT/f'{label}-review-{i+101}.png')
            results['screens'].append({'theme':theme,'device':label,'question':i+101,'mode':'review'})
        page.close()
    browser.close()
