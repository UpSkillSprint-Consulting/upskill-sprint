...
Run: python tests/browser/mbb-batch03-render.py --out /tmp/mbb-render
...
def inline(match):
    ...
    if re.match(r'test-bank-(mbb|css|cmq|cqe)',path) or path in ['theme.js','test-bank-formulas.js','test-bank-tables.js','test-bank-mbb-batch3-ui.js']:
...
KEYS=[2,0,3,1,2,0,3,1,2,0,3,1,2,0,2,3,1,2,0,3,1,2,0,3,1]
...
with sync_playwright() as p:
    ...
    for device,width,height in [('desktop',1440,1000),('tablet',768,1024),('mobile',390,844)]:
        ...
        page.evaluate('''()=>{window.__TB.EXAMS.mbb.sets[2]=window.MBB_SET2_BATCHES[3];Math.random=()=>0.9999999}''')
        ...
        for index in range(25):
            ...
            check(qid==f'mbb:set-2:original-{index+51:03}',f'{device} Q{index+51} identity')
            check(page.locator('.tb-opt').count()==4,f'{device} Q{index+51} four choices')
            check(page.locator('.tb-review-panel').count()==0,f'{device} Q{index+51} no pre-submit key/review')
            if not args.baseline:
                check(page.locator('.mbb3-conditions').count()==1,f'{device} Q{index+51} conditions visible')
            screenshot_element(page,'#tb-overview',OUT/f'{device}-{index+51:02}.png')
            metrics=page.evaluate('''()=>({bodyWidth:document.documentElement.scrollWidth,viewport:innerWidth,svgFontSizes:[...document.querySelectorAll('.tb-quiz svg text')].map(x=>parseFloat(getComputedStyle(x).fontSize)*(x.ownerSVGElement.getBoundingClientRect().width/x.ownerSVGElement.viewBox.baseVal.width)),tableCells:[...document.querySelectorAll('.tb-quiz .tb-q-data-table th,.tb-quiz .tb-q-data-table td')].map(x=>({text:x.textContent,width:x.getBoundingClientRect().width,height:x.getBoundingClientRect().height,fontSize:parseFloat(getComputedStyle(x).fontSize)})),touchHeights:[...document.querySelectorAll('.tb-opt,.mbb3-inspector select')].map(x=>x.getBoundingClientRect().height)})''')
            check(metrics['bodyWidth']<=width+1,f'{device} Q{index+51} no page overflow')
            if not args.baseline and metrics['svgFontSizes']:check(min(metrics['svgFontSizes'])>=11.9,f'{device} Q{index+51} SVG labels at least 12px')
            if not args.baseline and metrics['tableCells']:check(min(x['fontSize'] for x in metrics['tableCells'])>=13.9,f'{device} Q{index+51} table text at least 14px')
            results['screens'].append({'device':device,'qid':qid,'question':index+51,**metrics})
            if not args.baseline and index in [5,20]:
                select=page.locator('[data-mbb3-observation]');select.select_option('1');check(len(page.locator('[data-mbb3-readout]').inner_text())>10,f'{device} Q{index+51} touch/select readout')
                point=page.locator('[data-mbb3-point="0"]');point.focus();point.press('Enter');check(select.input_value()=='0',f'{device} Q{index+51} keyboard point readout')
                page.locator('.mbb3-data summary').click();check(page.locator('.mbb3-data tbody tr').count()==(8 if index==5 else 10),f'{device} Q{index+51} data alternative')
            if not args.baseline:
                for scroll in page.locator('.mbb3-scroll').all():
                    scroll.evaluate('x=>x.scrollLeft=x.scrollWidth');check(scroll.evaluate('x=>x.scrollWidth<=x.clientWidth || x.scrollLeft>0'),f'{device} Q{index+51} evidence scroll reachable');scroll.evaluate('x=>x.scrollLeft=0')
            if not args.baseline and index==10:
                slider=page.locator('[data-mbb3-capacity]');slider.fill('16');slider.dispatch_event('input')
                check('12 BB-months' in page.locator('[data-mbb3-capacity-readout]').inner_text(),device+' Q61 what-if changes remaining capacity')
                page.locator('[data-mbb3-reset]').click();check(slider.input_value()=='12',device+' Q61 resets to scored case')
            # Exercise every option and confirm the final choice is saved across re-render/navigation.
            for option in range(4):
                page.locator(f'[data-opt="{option}"]').click();check(page.locator(f'.tb-opt.sel[data-opt="{option}"]').count()==1,f'{device} Q{index+51} option {option} selection')
            page.locator(f'[data-opt="{KEYS[index]}"]').click()
            page.locator('[data-flag]').click();check('Flagged' in page.locator('[data-flag]').inner_text(),f'{device} Q{index+51} flag')
            page.locator('[data-flag]').click()
        page.locator('[data-goto="0"]').click();check(page.locator('.tb-opt.sel[data-opt="2"]').count()==1,device+' answer retained after navigation')
        ...
        for index in range(25):
            ...
            check(card.get_attribute('data-question-id')==f'mbb:set-2:original-{index+51:03}',f'{device} review Q{index+51} identity')
            if not args.baseline:
                check(card.locator('.mbb3-conditions').count()==1,f'{device} review Q{index+51} conditions')
                check(card.locator('.mbb3-rationales dt').count()==4,f'{device} review Q{index+51} all distractors explained')
                check(card.locator('.mbb3-evidence').count()==(1 if index+51 in [51,56,61,65,69,70,71,72,73,74] else 0),f'{device} review Q{index+51} evidence retained')
            check(page.evaluate('document.documentElement.scrollWidth<=innerWidth+1'),f'{device} review Q{index+51} no page overflow')
            screenshot_element(page,'.tb-review-card',OUT/f'{device}-review-{index+51:02}.png')
            results['screens'].append({'device':device,'question':index+51,'mode':'review'})
        ...
        page.evaluate("()=>{window.__TB.EXAMS.mbb.sets[2]=window.MBB_SET2_BATCHES[3].filter(q=>[56,61,71].includes(Number(q.qid.slice(-3))));Math.random=()=>0.9999999}")
        page.locator('[data-mode="full"]').click()
        for index,key in enumerate([0,3,1]):
        ...
        for index,key in enumerate([0,3,1]):
            if not args.baseline:
                check(page.locator('.mbb3-evidence').count()==1,f'{device} retry {index} evidence retained')
                check(page.locator('.mbb3-rationales').count()==0,f'{device} retry {index} no premature rationales')
            if index==1:
                slider=page.locator('[data-mbb3-capacity]');slider.fill('16');slider.dispatch_event('input');check('12 BB-months' in page.locator('[data-mbb3-capacity-readout]').inner_text(),device+' retry Q61 capacity');page.locator('[data-mbb3-reset]').click()
            else:
                page.locator('[data-mbb3-observation]').select_option('1');check(len(page.locator('[data-mbb3-readout]').inner_text())>10,device+' retry observation readout')
            page.locator(f'[data-retry-opt="{key}"]').click();page.locator('[data-retry-check]').click()
            if not args.baseline:
                check(page.locator('.mbb3-rationales dt').count()==4,f'{device} retry {index} checked rationales')
            ...
            screenshot_element(page,'#tb-overview',OUT/f'{device}-retry-{[56,61,71][index]}.png')
            results['screens'].append({'device':device,'question':[56,61,71][index],'mode':'retry'})
