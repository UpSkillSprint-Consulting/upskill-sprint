...
Run: python tests/browser/mbb-batch07-render.py --out /tmp/mbb-render
...
def inline(match):
    ...
    if re.match(r'test-bank-(mbb|css|cmq|cqe)',path) or path in ['theme.js','test-bank-formulas.js','test-bank-tables.js','test-bank-mbb-batch7-ui.js']:
...
with sync_playwright() as p:
    ...
    for device,width,height in [('desktop',1440,1000),('tablet',768,1024),('mobile',390,844)]:
        ...
        page.evaluate('''()=>{window.__TB.EXAMS.mbb.sets[2]=window.MBB_SET2_BATCHES[7];Math.random=()=>0.9999999}''')
        ...
        for index in range(25):
            ...
            check(qid==f'mbb:set-2:original-{index+151:03}',f'{device} Q{index+151} identity')
            check(page.locator('.tb-opt').count()==4,f'{device} Q{index+151} four choices')
            check(page.locator('.tb-review-panel').count()==0,f'{device} Q{index+151} no pre-submit key/review')
            if not args.baseline:
                check(page.locator('.mbb7-conditions').count()==1,f'{device} Q{index+151} conditions visible')
            screenshot_element(page,'#tb-overview',OUT/f'{device}-{index+151:02}.png')
            metrics=page.evaluate('''()=>({bodyWidth:document.documentElement.scrollWidth,viewport:innerWidth,svgFontSizes:[...document.querySelectorAll('.tb-quiz svg text')].map(x=>parseFloat(getComputedStyle(x).fontSize)*(x.ownerSVGElement.getBoundingClientRect().width/x.ownerSVGElement.viewBox.baseVal.width)),tableCells:[...document.querySelectorAll('.tb-quiz .tb-q-data-table th,.tb-quiz .tb-q-data-table td')].map(x=>({text:x.textContent,width:x.getBoundingClientRect().width,height:x.getBoundingClientRect().height,fontSize:parseFloat(getComputedStyle(x).fontSize)})),touchHeights:[...document.querySelectorAll('.tb-opt,.mbb7-inspector select')].map(x=>x.getBoundingClientRect().height)})''')
            results['assertions'].append({'label':f'{device} Q{index+151} page containment', 'passed':metrics['bodyWidth']<=width+1})
            if not args.baseline and metrics['svgFontSizes']:check(min(metrics['svgFontSizes'])>=11.9,f'{device} Q{index+151} SVG labels at least 12px')
            if not args.baseline and metrics['tableCells']:check(min(x['fontSize'] for x in metrics['tableCells'])>=13.9,f'{device} Q{index+151} table text at least 14px')
            results['screens'].append({'device':device,'qid':qid,'question':index+151,'mode':'attempt',**metrics})
            if not args.baseline and index in [7,19,23]:
                select=page.locator('[data-mbb7-observation]');select.select_option('1');check(len(page.locator('[data-mbb7-readout]').inner_text())>10,f'{device} Q{index+151} touch/select readout')
                point=page.locator('[data-mbb7-point="0"]');point.focus();point.press('Enter');check(select.input_value()=='0',f'{device} Q{index+151} keyboard point readout')
                page.locator('.mbb7-data summary').first.click();check(page.locator('.mbb7-data').first.locator('tbody tr').count()==({7:8,19:9,23:2}[index]),f'{device} Q{index+151} data alternative')
            if not args.baseline and index==4:
                slider=page.locator('[data-mbb7-capacity]');slider.fill('11');slider.dispatch_event('input')
                check('11 Belt-months' in page.locator('[data-mbb7-capacity-output]').inner_text(),device+' capacity what-if changes')
                page.locator('[data-mbb7-reset]').click();check(slider.input_value()=='10',device+' reset restores scored capacity')
                check('Scored case: 10' in page.locator('.mbb7-capacity').inner_text(),device+' baseline remains explicit')
            if not args.baseline:
                for scroll in page.locator('.mbb7-scroll').all():
                    scroll.evaluate('x=>x.scrollLeft=x.scrollWidth');check(scroll.evaluate('x=>x.scrollWidth<=x.clientWidth || x.scrollLeft>0'),f'{device} Q{index+151} evidence scroll reachable');scroll.evaluate('x=>x.scrollLeft=0')
            # Exercise every option and confirm the final choice is saved across re-render/navigation.
            for option in range(4):
                page.locator(f'[data-opt="{option}"]').click();check(page.locator(f'.tb-opt.sel[data-opt="{option}"]').count()==1,f'{device} Q{index+151} option {option} selection')
            page.locator(f'[data-opt="{KEYS[index]}"]').click()
            page.locator('[data-flag]').click();check('Flagged' in page.locator('[data-flag]').inner_text(),f'{device} Q{index+151} flag')
            page.locator('[data-flag]').click()
        page.locator('[data-goto="0"]').click();check(page.locator('.tb-opt.sel[data-opt="3"]').count()==1,device+' answer retained after navigation')
        ...
        for index in range(25):
            ...
            check(card.get_attribute('data-question-id')==f'mbb:set-2:original-{index+151:03}',f'{device} review Q{index+151} identity')
            if not args.baseline:
                check(card.locator('.mbb7-conditions').count()==1,f'{device} review Q{index+151} conditions')
                check(card.locator('.mbb7-rationales dt').count()==4,f'{device} review Q{index+151} all distractors explained')
                check(card.locator('.tb-exam-trap').inner_text()==page.evaluate('(i)=>MBB_SET2_BATCHES[7][i].trap',index),f'{device} review Q{index+151} explicit exam guidance')
                check(card.locator('.mbb7-evidence').count()==(1 if index+151 in [151,155,158,161,163,168,170,172,174] else 0),f'{device} review Q{index+151} evidence retained')
            results['assertions'].append({'label':f'{device} review Q{index+151} page containment','passed':page.evaluate('document.documentElement.scrollWidth<=innerWidth+1')})
            screenshot_element(page,'.tb-review-card',OUT/f'{device}-review-{index+151:02}.png')
            results['screens'].append({'device':device,'question':index+151,'mode':'review'})
        # Test the actual missed-question retry with all four Batch 7 interactions.
        page.set_content(raw,wait_until='load')
        page.locator('.tb-tile[data-exam="mbb"]').click();page.locator('.tb-setpick [data-set="2"]').click()
        page.evaluate("()=>{window.__TB.EXAMS.mbb.sets[2]=window.MBB_SET2_BATCHES[7].filter(q=>[155,158,170,174].includes(Number(q.qid.slice(-3))));Math.random=()=>0.9999999}")
        page.locator('[data-mode="full"]').click()
        for index,key in enumerate([3,2,2,2]):
        ...
        for index,key in enumerate([3,2,2,2]):
            check(page.locator('.mbb7-evidence').count()==1,f'{device} retry {index} evidence retained')
            check(page.locator('.mbb7-rationales').count()==0,f'{device} retry {index} no premature rationales')
            if index==0:
                slider=page.locator('[data-mbb7-capacity]');slider.fill('8');slider.dispatch_event('input')
                check('8 Belt-months' in page.locator('[data-mbb7-capacity-output]').inner_text(),device+' retry capacity works')
                page.locator('[data-mbb7-reset]').click();check(slider.input_value()=='10',device+' retry resets scored capacity')
            else:
                select=page.locator('[data-mbb7-observation]');select.select_option('1')
                check(len(page.locator('[data-mbb7-readout]').inner_text())>10,f'{device} retry {index} observation selection')
            page.locator(f'[data-retry-opt="{key}"]').click();page.locator('[data-retry-check]').click()
            check(page.locator('.mbb7-rationales dt').count()==4,f'{device} retry {index} checked rationales')
            ...
            screenshot_element(page,'#tb-overview',OUT/f'{device}-retry-{[155,158,170,174][index]}.png')
            results['screens'].append({'device':device,'question':[155,158,170,174][index],'mode':'retry'})
            page.locator('[data-retry-next]').click()
        check('4' in page.locator('#tb-overview').inner_text(),device+' retry completion')
