...
test('session trend preserves stored history while answer activity never fabricates dates from completion timestamps', async () => {
  const { window } = await load();
  const day = 86400000;
...
test('studyHeatmap counts final submitted answer events rather than planned blanks or completion time', async () => {
  const { window } = await load();
