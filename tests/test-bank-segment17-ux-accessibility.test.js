...
test('dynamic analytics exposes tablist relationships, roving focus, progress values and chart labels',async t=>{
  const dom=fixture();t.after(()=>dom.window.close());await settle(dom.window);
...
test('analytics tabs support Arrow/Home/End keyboard navigation without inventing a second state model',async t=>{
  const dom=fixture();t.after(()=>dom.window.close());await settle(dom.window);const w=dom.window,tabs=[...w.document.querySelectorAll('[data-analytics-tab]')];
...
test('non-native quiz navigation gets keyboard activation while native choices stay native',async t=>{
  const dom=fixture();t.after(()=>dom.window.close());await settle(dom.window);const w=dom.window,d=w.document,nav=d.querySelector('.tb-navcell'),choice=d.querySelector('[data-opt]');
...
test('critical New-only, sync, handoff and timing conditions have separate polite/assertive live regions',async t=>{
  const dom=fixture();t.after(()=>dom.window.close());await settle(dom.window);const w=dom.window,d=w.document;
...
test('focus visibility, touch target, reduced-motion, forced-color and print safeguards are explicit',()=>{
  assert.match(source,/:focus-visible/);assert.match(source,/min-height:44px/);assert.match(source,/prefers-reduced-motion:reduce/);assert.match(source,/forced-colors:active/);assert.match(source,/@media print/);
});
