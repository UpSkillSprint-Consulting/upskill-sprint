(function () {
  ...
  var FORMULAS = [
    ...
    f('reliability-function-definition', ['design', 'risk'], 'Reliability and cumulative-failure function definitions',
      'R(t) = P(survive past time t);  F(t) = 1 − R(t) = P(fail by time t)',
      'Use when a question gives a numeric reliability function value (e.g. R(t) = 998/1,000) and asks what it represents or asks for the corresponding probability of failure, without requiring a distribution-specific calculation.',
      'R(t) is the fraction of the population surviving past t; F(t) is the fraction that has failed by t.',
      'R(t) and F(t) are complements of each other (R(t) + F(t) = 1); do not confuse "survives past t" with "survives up to and including t."',
      [/reliability function.*(?:defined as|at time t)|R\(t\)\s*=|cumulative distribution function of failure/i], ['reliability function', 'R(t)']),
    ...
    f('xbar-r', ['ppc', 'quant'], 'X̄ and R chart limits',
    ...
    f('i-mr', ['ppc', 'quant'], 'Individuals and moving-range chart limits',
    ...
    f('p-chart', ['ppc', 'quant'], 'p-chart limits',
      ...
      [/\bp[ -]chart\b|p̄\s*=|p-bar|UCL.*p̄|LCL.*p̄|fraction nonconforming|proportion nonconforming|varying sample size.*defective|fraction defective/i], ['proportion chart']),
    f('np-chart', ['ppc', 'quant'], 'np-chart limits',
    ...
    f('c-chart', ['ppc', 'quant'], 'c-chart limits',
    ...
    f('u-chart', ['ppc', 'quant'], 'u-chart limits',
    ...
    f('ansi-asq-sampling-tables', ['ppc'], 'ANSI/ASQ Z1.4 and Z1.9 sample-size lookup',
      'Sample size code letter ← lot size + inspection level (Table 1); then n, Ac, Re ← code letter + AQL (Table II-A / Table B-3)',
      'Use when a lot size and AQL are given under ANSI/ASQ Z1.4-2003 (attributes) or Z1.9-2003 (variables) and the question asks for the required sample size, acceptance number, or rejection number.',
      'Lot size and inspection level determine the sample size code letter; the code letter and AQL then determine the actual sample size, Ac, and Re from the standard\u2019s tables.',
      'These are table lookups, not closed-form calculations \u2014 different code letters at low AQLs can map to the same sample size.',
      [/ANSI\/ASQ Z1\.4|ANSI\/ASQ Z1\.9|sample size code letter|required sample size.*(?:sampling plan|inspection)|AQL\s*(?:is|=)/i], ['Z1.4', 'Z1.9', 'AQL sampling tables']),
    ...
    f('z-score', ['quant'], 'Standard score',
      'z = (x − μ) / σ',
      'Use to express how many population standard deviations an observation lies from the mean, or to find the probability that a standard normal variable falls in a given range.',
      ...
      [/z[- ]score|standard score|standard deviations.*from.*mean|normal probability|standard normal distribution.*lies between|probability that z/i], ['z value']),
    f('hypothesis-test-power', ['quant'], 'Power and Type I / Type II error of a hypothesis test',
      'Power = 1 − β = P(correctly reject a false H₀);  α = P(Type I error);  β = P(Type II error)',
      'Use when a question asks for the power of a test, or defines/relates Type I error, Type II error, significance level, or power to each other.',
      'α = significance level = P(reject H₀ | H₀ true); β = P(fail to reject H₀ | H₀ false); Power = 1 − β.',
      'Type I error rejects a true H₀; Type II error fails to reject a false H₀ — they are not interchangeable, and power is defined in terms of β, not α.',
      [/type i error|type ii error|power (?:of|for) (?:this|the|a) (?:hypothesis )?test|correctly reject(?:ing)? a false/i], ['power of a test', 'beta risk']),
    f('chi-square-variance-test', ['quant'], 'Hypothesis test for a single population variance',
      'χ₀² = (n − 1)s² / σ₀²',
      'Use when a hypothesis test concerns a single population variance (or standard deviation) against a hypothesized value.',
      'n = sample size; s² = sample variance; σ₀² = hypothesized population variance.',
      'The test statistic follows a chi-square distribution with n − 1 degrees of freedom; watch for one-tailed vs. two-tailed critical values.',
      [/hypothesis test.*variance|test.*(?:the )?variance.*(?:is|less than|greater than|equal)/i], ['chi-square test for variance']),
    f('six-sigma-dpmo', ['quant', 'ci'], 'Six Sigma quality level and defects per million opportunities',
      'At Six Sigma with the standard 1.5σ shift: 3.4 DPMO; without the shift: ~0.002 DPMO',
      'Use when a question asks for the defect rate or DPMO associated with a stated sigma quality level, especially referencing the 1.5σ shift.',
      'DPMO = defects per million opportunities; the 1.5σ shift is the standard long-term process-shift assumption in Six Sigma.',
      'The famous "3.4 defects per million" figure already accounts for the 1.5σ shift — without it, Six Sigma quality is far tighter (~0.002 DPMO).',
      [/1\.5.{0,3}shift|sigma.{0,10}quality.{0,15}(?:defective|dpmo|defects per million)|defects per million.{0,20}sigma/i], ['DPMO', 'sigma shift']),
    f('probability-addition-rule', ['quant'], 'Addition rule of probability',
      'P(A ∪ B) = P(A) + P(B) − P(A ∩ B);  if A, B mutually exclusive: P(A ∪ B) = P(A) + P(B)',
      'Use when a question asks for the probability that at least one of two events (A or B) occurs.',
      'For mutually exclusive events, P(A ∩ B) = 0, so the joint-probability term drops out.',
      'Do not add probabilities directly unless the events are confirmed mutually exclusive; otherwise the intersection must be subtracted.',
      [/probability that either|either .* or .* occurs|mutually exclusive/i], ['union of events', 'OR probability']),
    ...
    f('conditional-probability', ['quant', 'risk'], 'Conditional probability',
      'P(A|B) = P(A ∩ B) / P(B)',
      'Use when a question asks for the probability of one event given that another has occurred, including reading probabilities directly from a contingency table.',
      'P(A ∩ B) = joint probability of both events; P(B) = probability of the given (conditioning) event.',
      'Divide the joint probability by the probability of the condition, not by the overall total.',
      [/given that.*(?:what is the probability|probability it is)|probability.*given that|contingency table.*probability/i], ['conditional probability']),
    f('expected-value', ['quant', 'risk'], 'Expected value of a discrete random variable',
      'E(X) = Σ xᵢ P(X = xᵢ)',
      'Use when a probability mass function (a table of outcomes and their probabilities) is given and the question asks for the expected or average value.',
      'xᵢ = each possible outcome; P(X = xᵢ) = the probability of that outcome.',
      'Every probability in the table must be multiplied by its own outcome value, then summed — not averaged as if outcomes were equally likely.',
      [/probability mass function|expected number of|expected value.*random variable/i], ['expected value', 'mean of a discrete distribution']),
    f('p-value-decision', ['quant'], 'p-value hypothesis-test decision rule',
      'Reject H₀ if p-value < α',
      'Use when a hypothesis test gives a p-value and the question asks for the conclusion at a stated significance level.',
      'α = significance level; the p-value is compared directly to α, not to the test statistic.',
      'A small p-value rejects H₀ in favor of Hₐ; it does not by itself say how large or practically important the effect is.',
      [/p-value.*(?:significance level|conclusion|hypothesis test)|value of the hypothesis test.*conclusion/i], ['p-value decision rule']),
