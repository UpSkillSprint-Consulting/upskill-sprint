...
<section id="spx-tab-hardenability" class="spx-panel" role="tabpanel" data-panel="hardenability" hidden>
  ...
  <div class="spx-grid-2 spx-r2-grid">
    <div class="spx-card">
      ...
      <div class="spx-svg-wrap" role="region" aria-label="Scrollable Jominy hardenability chart" tabindex="0"><svg id="spx-jominy-svg" viewBox="0 0 760 410" role="img" aria-label="Estimated Jominy hardness and martensite profile"></svg></div>
    </div>
    <div class="spx-side-stack">
      ...
      <div class="spx-card spx-card-pad"><div class="spx-card-title"><div><h2>Hardness through the section</h2><p>Surface, quarter-depth, and centre estimates.</p></div></div><div class="spx-r2-canvas-scroll" role="region" aria-label="Scrollable section hardness chart" tabindex="0"><canvas id="spx-section-canvas" width="520" height="250" role="img" aria-label="Section hardness gradient"></canvas></div><div class="spx-table-wrap"><table class="spx-table"><thead><tr><th>Location</th><th>Equivalent Jominy</th><th>Martensite</th><th>Hardness</th></tr></thead><tbody id="spx-hard-depth-body"></tbody></table></div></div>
...
<section id="spx-tab-austenitization" class="spx-panel" role="tabpanel" data-panel="austenitization" hidden>
  ...
  <div class="spx-grid-2 spx-r2-grid">
    <div class="spx-card">
      <div class="spx-card-pad"><div class="spx-card-title"><div><h2>Cycle definition</h2><p>Temperature is compared with the relevant full-austenite reference: Andrews Ac₃ below the eutectoid, or the plain-carbon Acm equilibrium boundary above it.</p></div><button class="spx-btn primary" id="spx-aust-set-ac3" type="button">Set to full-austenite reference + 40°</button></div>
        <div class="spx-fields spx-r2-fields">
      ...
      <div class="spx-r2-canvas-wrap spx-r2-canvas-scroll" role="region" aria-label="Scrollable austenitization operating-window chart" tabindex="0"><canvas id="spx-aust-window-canvas" width="760" height="390" role="img" aria-label="Austenitization operating window heatmap"></canvas></div>
    </div>
    <div class="spx-side-stack">
      ...
      <div class="spx-card spx-card-pad"><div class="spx-card-title"><div><h2>Prior-austenite grain estimate</h2><p>Schematic comparison, not a metallograph.</p></div></div><div class="spx-r2-canvas-scroll" role="region" aria-label="Scrollable prior-austenite grain comparison" tabindex="0"><canvas id="spx-grain-canvas" width="520" height="240" role="img" aria-label="Initial and estimated final grain structure"></canvas></div><div id="spx-grain-summary" class="spx-note" style="margin-top:9px"></div></div>
...
<section id="spx-tab-quenching" class="spx-panel" role="tabpanel" data-panel="quenching" hidden>
  <div class="spx-r2-hero spx-card spx-card-pad"><p class="spx-eyebrow">Release 2 · Quenching</p><div class="spx-card-title"><div><h2>Quench-medium and cracking-risk comparison</h2><p>Compare directional cooling, transformation, hardness, distortion, and cracking tendencies.</p></div><button class="spx-btn" type="button" data-r2-help="quenching">How to interpret</button></div><div class="spx-note"><strong>Model caution:</strong> cooling rates, martensite fractions, hardness values, and risk scores are uncalibrated teaching estimates. Results are limited by the current austenitization estimate; invalid medium/temperature combinations are reported as unavailable.</div><div class="spx-note spx-r2-safety"><strong>Safety:</strong> this educational module does not provide operating instructions for hot oils, brines, polymers, molten salts, or high-temperature handling. Follow approved industrial procedures, SDS requirements, and engineered safeguards.</div></div>
  <div class="spx-grid-2 spx-r2-grid">
    <div class="spx-card">
      <div class="spx-card-pad"><div class="spx-card-title"><div><h2>Quench setup</h2><p>Surface and centre cooling are estimated separately.</p></div><button class="spx-btn primary" id="spx-quench-apply-hard" type="button">Apply to hardenability tab</button></div>
        <div class="spx-fields spx-r2-fields">
          ...
          <div class="spx-field"><label for="spx-quench-final">Martensite assessment temperature <span class="spx-hint spx-r2-temp-unit">(°C)</span></label><input id="spx-quench-final" type="number" step="5" value="25"></div>
        </div>
      </div>
      <div class="spx-svg-wrap" role="region" aria-label="Scrollable quench cooling chart" tabindex="0"><svg id="spx-quench-svg" viewBox="0 0 760 430" role="img" aria-label="Estimated surface quarter-depth and centre cooling curves"></svg></div>
    </div>
    <div class="spx-side-stack">
      <div class="spx-card spx-card-pad"><div class="spx-card-title"><div><h2>Selected-quench result</h2><p id="spx-quench-selected-label">Heuristic result pending</p></div></div><div id="spx-quench-metrics" class="spx-metrics"></div><div id="spx-quench-risks" class="spx-r2-risk-grid"></div><div id="spx-quench-status" class="spx-note" style="margin-top:9px"></div></div>
      <div class="spx-card spx-card-pad"><div class="spx-card-title"><div><h2>Surface-to-centre outcome</h2><p>Heuristic martensite and hardness trend; not an acceptance prediction.</p></div></div><div class="spx-r2-canvas-scroll" role="region" aria-label="Scrollable quenched-section transformation chart" tabindex="0"><canvas id="spx-quench-section-canvas" width="520" height="240" role="img" aria-label="Quenched section transformation gradient"></canvas></div><div class="spx-table-wrap"><table class="spx-table"><thead><tr><th>Location</th><th>800→500 rate*</th><th>Martensite*</th><th>Hardness*</th></tr></thead><tbody id="spx-quench-depth-body"></tbody></table></div></div>
    </div>
  </div>
  <div class="spx-card spx-card-pad spx-release-section"><div class="spx-card-title"><div><h2>Normalized quench-medium comparison</h2><p>Illustrative severity comparison only. Medium-specific model windows are enforced; concentration, condition, and real boiling-stage behaviour require measured data.</p></div></div><div class="spx-table-wrap"><table class="spx-table"><thead><tr><th>Medium</th><th>Surface rate</th><th>Centre martensite</th><th>Centre hardness</th><th>Cracking index</th><th>Distortion index</th><th>Validity / use note</th></tr></thead><tbody id="spx-quench-compare-body"></tbody></table></div></div>
</section>`;
