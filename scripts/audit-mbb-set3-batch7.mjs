// Real-player audit of canonical Set 3 Q151–175, not a replacement renderer.
...
const root=process.cwd(),out=path.resolve(process.env.AUDIT_OUT||'audit-results-batch7');fs.mkdirSync(out,{recursive:true});
const sandbox={window:{}};vm.runInNewContext(fs.readFileSync('test-bank-mbb-set3.js','utf8'),sandbox);
const questions=JSON.parse(JSON.stringify(sandbox.window.MBB_SET3)).slice(150,175);
const report={interactions:[],scope:'Canonical Set 3 Q151–175',backend:'isolated authentication and remote-persistence fixture',cases:[],failures:[],pageErrors:[]};
...
async function checkVisual(page,q,host,phase,label){
 ...
 const rows=host.locator('.mbbs3b7-table tbody tr');assert.equal(await rows.count(),t.rows.length);
 for(let i=0;i<t.rows.length;i++)assert.deepEqual(await rows.nth(i).locator('th,td').allTextContents(),t.rows[i].map(String));
 assert.equal(await host.locator('.mbbs3b7-table thead th[scope="col"]').count(),t.columns.length);
 assert.equal(await host.locator('.mbbs3b7-table tbody th[scope="row"]').count(),t.rows.length);
 assert.equal(await host.locator('circle[data-point]').count(),0);
 ...
 const regions=host.locator('.mbbs3b7-scroll'),access=[];
 ...
 for(const layout of layouts){
  ...
  try{
   ...
   for(let i=0;i<questions.length;i++){
    const q=questions[i],index=order.indexOf(q.qid),label=engine+'-'+layout+'-q'+String(i+151).padStart(2,'0');
    report.current={engine,layout,phase:'question',n:i+151,qid:q.qid};report.operation=null;save();
    ...
    for(const theme of ['light','dark']){
     ...
     const g=await geometry(page,'.tb-quiz');const axe=await phase(`question-${i+151}-${theme}-axe`,()=>analyzeSingleDocument(page,AxeBuilder,'.tb-quiz'));
     report.cases.push({engine,layout,theme,number:i+151,qid:q.qid,phase:'question',fourChoicesSelected:true,keyboardSpace:true,reopenedSelection:true,geometry:g,navigation,a11y:{completed:true,engine:axe.testEngine,passedRules:axe.passes.length,incompleteRules:axe.incomplete.map(r=>r.id)},axe:axe.violations.map(v=>({id:v.id,impact:v.impact,nodes:v.nodes.map(n=>({target:n.target,summary:n.failureSummary}))}))});save();
     if(layout==='mobile'&&q.chart){const area=page.locator('.mbbs3b7-scroll').first();const scroll=await area.evaluate(e=>{e.scrollLeft=e.scrollWidth;return {needed:e.scrollWidth>e.clientWidth+2,moved:e.scrollLeft>0};});assert.ok(!scroll.needed||scroll.moved);await area.screenshot({path:path.join(out,label+'-'+theme+'-visual-right.png')});await area.evaluate(e=>e.scrollLeft=0);}
    }
   ...
   for(let i=0;i<questions.length;i++){
    const q=questions[i],index=order.indexOf(q.qid);report.current={engine,layout,phase:'review',n:i+151,qid:q.qid};report.operation=null;save();await page.locator('[data-review-goto="'+index+'"]').click();
