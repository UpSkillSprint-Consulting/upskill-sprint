...
const root=process.cwd(),out=path.resolve(process.env.AUDIT_OUT||'audit-results-batch6');fs.mkdirSync(out,{recursive:true});
 ...
 for(const layout of layouts){
  ...
  try{
   ...
   for(let i=0;i<questions.length;i++){
    ...
    report.current={engine,layout,phase:'question',n:i+126,qid:q.qid};report.operation=null;save();
    ...
    for(const theme of ['light','dark']){
     ...
     const g=await geometry(page,'.tb-quiz');const axe=await phase(`question-${i+126}-${theme}-axe`,()=>analyzeSingleDocument(page,AxeBuilder,'.tb-quiz'));
     report.cases.push({engine,layout,theme,number:i+126,qid:q.qid,phase:'question',fourChoicesSelected:true,keyboardSpace:true,reopenedSelection:true,geometry:g,navigation,a11y:{completed:true,engine:axe.testEngine,passedRules:axe.passes.length,incompleteRules:axe.incomplete.map(r=>r.id)},axe:axe.violations.map(v=>({id:v.id,impact:v.impact,nodes:v.nodes.map(n=>({target:n.target,summary:n.failureSummary}))}))});save();
   ...
   for(let i=0;i<questions.length;i++){
    const q=questions[i],index=order.indexOf(q.qid);report.current={engine,layout,phase:'review',n:i+126,qid:q.qid};report.operation=null;save();await page.locator('[data-review-goto="'+index+'"]').click();
    ...
    for(const theme of ['light','dark']){
     ...
     const axe=await phase(`review-${i+126}-${theme}-axe`,()=>analyzeSingleDocument(page,AxeBuilder,'.tb-review-card'));
