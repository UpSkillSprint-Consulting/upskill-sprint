...
const {bounded,analyzeSingleDocument,verifyCoverage,publishProgress,atomicJson}=runtime;
...
const root=process.cwd(),out=path.resolve(root,process.env.AUDIT_OUT||'audit-results-final175');fs.mkdirSync(out,{recursive:true});
...
const report={schemaVersion:1,profile:process.env.AUDIT_PROFILE||'local',engine,layout,width:layout==='desktop'?1440:Number(process.env.AUDIT_WIDTH)||390,mixed,accessibilityRequired:process.env.AUDIT_AXE==='1',accessibilityScans:0,toolVersions:{playwright:tools('playwright/package.json').version,axe:tools('axe-core').version,node:process.version},source:process.env.AUDIT_SOURCE||'',questions:[],reviews:[],visuals:[],failures:[],pageErrors:[],timings:[],scope:'All 175 canonical Set 3 items in one uninterrupted session',backend:'isolated fixtures'};
const save=()=>{atomicJson(path.join(out,'report.json'),report);publishProgress(out,report);};
const phase=(label,fn,ms=60000)=>bounded(label,fn,ms,s=>{report.operation={...report.current,...s};if(s.status==='passed'){report.phaseStats||={completed:0,slowest:null};report.phaseStats.completed++;if(!report.phaseStats.slowest||s.elapsedMs>report.phaseStats.slowest.elapsedMs)report.phaseStats.slowest={...report.operation};}publishProgress(out,report);if(s.status==='failed'){report.failedOperation=report.operation;save();}});
...
let browser,context,page;try{
 browser=await phase('browser-launch',()=>({chromium,webkit}[engine]).launch());report.browserVersion=browser.version();
 if(report.accessibilityRequired){
 ...
 await phase('initial-ledger-hydration',()=>page.evaluate(async()=>{await __TBLearning.sync('test-hydrate');await new Promise(r=>setTimeout(r,0));const f=__TBLearning.startSession;__TBLearning.startSession=function(c){window.__AUDIT_ORDER=c.questions.map(q=>q.qid);return f.call(this,c);};}));
 await page.locator('.tb-tile[data-exam="mbb"]').click();await page.locator('.tb-setpick [data-set="3"]').click();await page.locator('[data-mode="full"]').click();await page.locator('.tb-quiz').waitFor();const order=await page.evaluate(()=>__AUDIT_ORDER);report.sessionOrder=order;assert.equal(new Set(order).size,175);const clock=await page.locator('#tb-timer').innerText();await page.waitForTimeout(1150);assert.notEqual(await page.locator('#tb-timer').innerText(),clock);
 ...
 await page.locator('[data-goto="174"]').click();let t=Date.now();await phase('submit-175',()=>page.locator('[data-submit]').click());await page.locator('[data-open-review="all"]').waitFor();report.timings.push({action:'submit175',ms:Date.now()-t});const correct=mixed?59:175;report.expectedCorrect=correct;assert.match(await page.locator('.tb-resverd').innerText(),new RegExp(correct+' of 175 correctly'));report.verdict=await page.locator('.tb-resverd').innerText();
 t=Date.now();await phase('open-full-review',async()=>{await page.locator('[data-open-review="all"]').click();await page.locator('.tb-review-card').first().waitFor();});report.timings.push({action:'openFullReview',ms:Date.now()-t});assert.equal(await page.locator('.tb-review-card').count(),175);
