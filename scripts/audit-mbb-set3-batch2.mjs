// Real-player audit of canonical Set 3 Q26–50, not a replacement renderer.
...
const root=process.cwd(),out=path.resolve(process.env.AUDIT_OUT||'audit-results-batch2');fs.mkdirSync(out,{recursive:true});
const sandbox={window:{}};vm.runInNewContext(fs.readFileSync('test-bank-mbb-set3.js','utf8'),sandbox);
const questions=JSON.parse(JSON.stringify(sandbox.window.MBB_SET3)).slice(25,50);
const report={scope:'Canonical Set 3 Q26–50',backend:'isolated authentication and remote-persistence fixture',cases:[],failures:[],pageErrors:[]};
 ...
 for(const layout of layouts){
  ...
  try{
   ...
   for(let i=0;i<questions.length;i++){
    const q=questions[i],index=order.indexOf(q.qid),label=engine+'-'+layout+'-q'+String(i+26).padStart(2,'0');
    report.current={engine,layout,phase:'question',n:i+26,qid:q.qid};report.operation=null;save();
    ...
    for(const theme of ['light','dark']){
     ...
     const g=await geometry(page,'.tb-quiz');const axe=await phase(`question-${i+26}-${theme}-axe`,()=>analyzeSingleDocument(page,AxeBuilder,'.tb-quiz'));
     report.cases.push({engine,layout,theme,number:i+26,qid:q.qid,phase:'question',fourChoicesSelected:true,keyboardSpace:true,reopenedSelection:true,geometry:g,navigation,a11y:{completed:true,engine:axe.testEngine,passedRules:axe.passes.length,incompleteRules:axe.incomplete.map(r=>r.id)},axe:axe.violations.map(v=>({id:v.id,impact:v.impact,nodes:v.nodes.map(n=>({target:n.target,summary:n.failureSummary}))}))});save();
     if(layout==='mobile'&&q.chart){const area=page.locator('.mbbs3b2-scroll').first();const scroll=await area.evaluate(e=>{e.scrollLeft=e.scrollWidth;return {needed:e.scrollWidth>e.clientWidth+2,moved:e.scrollLeft>0};});assert.ok(!scroll.needed||scroll.moved);await area.screenshot({path:path.join(out,label+'-'+theme+'-visual-right.png')});await area.evaluate(e=>e.scrollLeft=0);}
    }
   ...
   for(let i=0;i<questions.length;i++){
    const q=questions[i],index=order.indexOf(q.qid);report.current={engine,layout,phase:'review',n:i+26,qid:q.qid};report.operation=null;save();await page.locator('[data-review-goto="'+index+'"]').click();
