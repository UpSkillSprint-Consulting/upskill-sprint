...
const root=process.cwd(),out=path.join(root,'audit-results-batch2');fs.mkdirSync(out,{recursive:true});
  ...
  try{
   ...
   for(let i=0;i<questions.length;i++){
    ...
    for(const theme of ['light','dark']){
     ...
     report.cases.push({engine,layout,theme,number:i+26,qid:q.qid,phase:'question',fourChoicesSelected:true,keyboardSpace:true,reopenedSelection:true,geometry:g,navigation,axe:axe.violations.map(v=>({id:v.id,impact:v.impact,nodes:v.nodes.map(n=>({target:n.target,summary:n.failureSummary}))}))});save();
