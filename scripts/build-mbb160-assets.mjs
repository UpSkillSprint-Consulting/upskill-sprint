...
function constructionSpec(question){
  ...
  if(chart.type==='activity-network')return {...common,variables:['activity','duration','predecessor-successor relationship'],units:question.batch===5?chart.durationUnit:'working days',layout:{nodes:chart.nodes,edges:chart.edges},calculations:[question.formula]};
...
for(const [batchKey,questions] of Object.entries(batches).sort(([left],[right])=>Number(left)-Number(right))){
  ...
  const finalAudited=questions[0].auditRevision==='final-audit-v1';
  if(finalAudited)dom.window.eval(fs.readFileSync(path.join(ROOT,'test-bank-mbb-final-audit-ui.js'),'utf8'));
  const finalStyles=finalAudited?':root{--ink:#16343e;--card:#fff;--paper:#f3f6f7;--line:#bcc9ce;--muted:#49616b;--teal:#137c83}html.dark,html[data-theme=dark]{--ink:#e7eef2;--card:#16262e;--paper:#20333e;--line:#566d79;--muted:#b9cbd4;--teal:#71d6d4}.fallback-card{background:var(--card)!important;color:var(--ink)}'+dom.window.__MBBFinalAuditUI.css:'';
  ...
  for(const question of visualQuestions){
    ...
    if(finalAudited)Object.assign(specs.questions[question.qid],{rendererVersion:'final-audit-v1',units:question.chart.durationUnit||question.chart.units||question.chart.responseUnit||question.chart.yLabel||'as labelled',accessibility:{altText:question.visual.altText,semanticTable:true},responsive:{minimumChartWidthPx:740,minimumTableWidthPx:700,boundedScrolling:true}});
    validation.questions[question.qid]={
      datasetSha256:hash,
      datasetMatchesQuestionChart:true,
      rendererReturnedMarkup:true,
      readableLabels:question.batch===5?'requires measured browser evidence':question.batch===3?'requires measured browser evidence':question.batch===4?'requires measured browser evidence':question.batch===1?null:question.batch===2?'requires browser evidence':true,
      scaleAndUnitsReviewed:question.batch===5?'see independent calculations and item tracker':question.batch===3?'see reconstructed calculations and integration tracker':question.batch===4?'see independent calculations and item tracker':question.batch===1?null:question.batch===2?'see question-level audit tracker':true,
      answerCueAuditPassed:true,
      accessibleAlternativePresent:Boolean(question.visual.altText),
      staticFallbackGenerated:true,
      breakpoints:[1,3,4,5].includes(batchNumber)?[]:['desktop','tablet','mobile'],
      ...
      validationStatus:[1,3,4,5].includes(batchNumber)?'semantic-checks-passed':'passed'
    };
    if(finalAudited)Object.assign(validation.questions[question.qid],{readableLabels:'see measured browser evidence',scaleAndUnitsReviewed:'see independent audit calculations',breakpoints:[],validationStatus:'semantic-checks-passed',validationScope:'Generated hashes and markup only; responsive measurements are in the batch audit report.'});
...
<html lang="en" data-theme="light"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ASQ MBB ${[2,5].includes(batchNumber)?'Set 2':'160'} - Batch ${batchNumber} visual fallbacks</title><script src="/theme.js"></script><script src="/site-sections.js"></script><style>${batchStyles}${batch3Styles}${batch5Styles}${finalStyles}
