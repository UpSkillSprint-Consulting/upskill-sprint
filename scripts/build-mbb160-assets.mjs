...
function constructionSpec(question){
  ...
  const common={
    ...
    transformations:[],
    referenceLines:[],
    responsive:/^mbb:set-2:original-(00[1-9]|01[0-9]|02[0-5])$/.test(question.qid) ? {container:'horizontal overflow for wide tables and SVGs; preserve legible labels',minimumChartWidthPx:560,viewBoxDriven:chart.type!=='data-table'} : {container:'horizontal overflow for wide tables; scalable SVG otherwise',minimumWidthPx:280,viewBoxDriven:chart.type!=='data-table'},
...
const batchArgument=process.argv.slice(2).find(value=>value.startsWith('--batch='));
const onlyBatch=batchArgument?Number(batchArgument.split('=')[1]):null;
if(onlyBatch!==null&&(!Number.isInteger(onlyBatch)||onlyBatch<1||onlyBatch>7))throw new Error('Use --batch=1 through --batch=7');
...
for(const [batchKey,questions] of Object.entries(batches).sort(([left],[right])=>Number(left)-Number(right))){
  ...
  if(onlyBatch!==null&&batchNumber!==onlyBatch)continue;
  ...
  for(const question of visualQuestions){
    ...
    validation.questions[question.qid]={
      ...
      readableLabels:batchNumber===1?null:true,
      scaleAndUnitsReviewed:batchNumber===1?null:true,
    ...
    const prompt=batchNumber===1?dom.window.__TBMbbBatch1Review.render(question,dom.window.__TB.renderQuestionChart,escapeHtml).replace(/<input[^>]*type="range"[^>]*>/g,''):`${rendered}<p class="fallback-stem">${escapeHtml(question.stem)}</p>`;
    cards.push(`<article class="fallback-card${batchNumber===1?' tb-mbb-batch1':''}" id="${anchor}" data-question-id="${escapeHtml(question.qid)}"><header><span>${escapeHtml(question.bok.domain)}</span><strong>${escapeHtml(question.qid)}</strong></header>${prompt}<ol class="fallback-options">${choices}</ol><details><summary>Accessible visual description</summary><p>${escapeHtml(question.visual.altText)}</p></details></article>`);
  }
...
<html lang="en" data-theme="light"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ASQ MBB 160 - Batch ${batchNumber} visual fallbacks</title><script src="/theme.js"></script><script src="/site-sections.js"></script><style>${batchStyles}
