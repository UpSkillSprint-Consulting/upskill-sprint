...
const OUT_DIR=path.join(ROOT,'test-bank-assets','mbb-160','batch-01');
...
function loadBank(){
  ...
  return JSON.parse(JSON.stringify(sandbox.MBB_SET2));
}
...
function constructionSpec(question){
  ...
  if(chart.type==='time-series')return {...common,variables:[chart.xLabel,chart.yLabel],units:'orders',observations:chart.data.length,order:'chronological',calculations:[]};
  return common;
}

const bank=loadBank();
const visualQuestions=bank.filter(question=>question.visual);
const dom=createRenderer();
const styles=[...dom.window.document.querySelectorAll('style')].map(style=>style.textContent).join('\n');

const datasets={schemaVersion:1,batch:1,generatedBy:'scripts/build-mbb160-assets.mjs',questions:{}};
const specs={schemaVersion:1,batch:1,generatedBy:'scripts/build-mbb160-assets.mjs',questions:{}};
const validation={schemaVersion:1,batch:1,generatedBy:'scripts/build-mbb160-assets.mjs',questions:{}};
const cards=[];
