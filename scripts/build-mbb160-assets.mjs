...
for(const [batchKey,questions] of Object.entries(batches).sort(([left],[right])=>Number(left)-Number(right))){
  ...
  for(const question of visualQuestions){
    ...
    validation.questions[question.qid]={
      ...
      readableLabels:question.batch===3?'requires measured browser evidence':question.batch===4?'requires measured browser evidence':question.batch===1?null:question.batch===2?'requires browser evidence':true,
      scaleAndUnitsReviewed:question.batch===3?'see reconstructed calculations and integration tracker':question.batch===4?'see independent calculations and item tracker':question.batch===1?null:question.batch===2?'see question-level audit tracker':true,
      answerCueAuditPassed:true,
      accessibleAlternativePresent:Boolean(question.visual.altText),
      staticFallbackGenerated:true,
      breakpoints:[1,3,4].includes(batchNumber)?[]:['desktop','tablet','mobile'],
      ...
      validationStatus:[1,3,4].includes(batchNumber)?'semantic-checks-passed':'passed'
    };
...
<html lang="en" data-theme="light"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ASQ MBB ${batchNumber===2?'Set 2':'160'} - Batch ${batchNumber} visual fallbacks</title><script src="/theme.js"></script><script src="/site-sections.js"></script><style>${batchStyles}${batch3Styles}
